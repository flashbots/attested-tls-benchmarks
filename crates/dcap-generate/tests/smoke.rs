...
fn benchmark_smoke_writes_csv() {
    ...
    Command::new("cargo")
        ...
        .args(["run", "-p", "dcap-generate-benchmark", "--"])
        .args([
            "--workload",
            "fs",
            "--concurrency",
...
#[test]
fn benchmark_verify_smoke_writes_csv() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let csv_path = tmp.path().join("verify-smoke.csv");
    ...
    let csv = fs::read_to_string(&csv_path).expect("csv should exist");
    assert!(csv.starts_with("timestamp,benchmark,workload,concurrency,ops,failures"));
    assert!(csv.contains("verify_dcap"));
    assert!(csv.contains(",verify,"));
}
